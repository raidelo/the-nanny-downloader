from pathlib import Path
from re import compile

TEMPLATE_URL = "https://retrotve.com/?trdownload=%(trdownload)s&trid=%(trid)s"
DEFAULT_USER_AGENT = "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Safari/537.36>"
DEFAULT_DOWNLOAD_FILENAME = "archivo_descargado"
CHAPTER_MATCH = compile(r"^(\d)x(\d{1,2})$")
TRID_MAPPING_PATH = Path(__file__).with_name("trid-mapping.json")
TRDOWNLOAD_MAPPING = {
    # None: 0, # TODO: add source
    # None: 1, # TODO: add source
    # None: 2, # TODO: add source
    "mediafire": 3,
    "mega": 4,
}
